...
Examples:
    ...
    Open port and set parameters:
            p = io.Serial:open('/dev/ttyUSB0', {baud=9600,bits=8,stops=0,parity='n'})
        or
...
io.Serial = {
    ...
    del_us = ul_time.delay_us, -- microseconds delay function
    ins = table.insert, rem = table.remove,
    ...
    -- open new port
    open = function(self, name, params)
        local port = nil

        if type(name) == 'number' then name = ports()[name] end
        
        local pd = io._serial.open(name) -- create port structure
        if pd ~= nil then
            port = {}
    ...
    parity = function(self, parity)
        if not parity then return io._serial.getParity(self.pd)
            ...
            io._serial.setParity(self.pd, parity)
            return self
        end
    end,
